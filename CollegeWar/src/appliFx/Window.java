 ...
 * @since 19/02/2019
 * @version 1.0
 * @see Manages the graphic interface for the standard mode
 */
public class Window extends Application {
	...
	public void start(Stage primaryStage) {
		...
		primaryStage.setOnCloseRequest(new EventHandler<WindowEvent>() {
			...
			public void handle(WindowEvent event) {
				...
				try {
					new PlayerBackup(a.getIndex(), a.getPlayer().getStatistics()).save();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
	...
	private void nextCard() {
		...
		appFX.setApplication(current.getBackground(), app.getPlayer().getStatistics(), app.getPlayer().getItems(),
